...
var ATL = (function () {
    ...
    ATL.prototype.fromAST = function (astRoot) {
        ...
        if (this.options.variables['ENV']) {
            var clonedEnv_1 = _.cloneDeep(process.env);
            Object.keys(this.options.variables['ENV']).forEach(function (key) {
                _this.options.variables['ENV'][key] = clonedEnv_1[key] || _this.options.variables['ENV'][key];
            });
        }
        ...
        if (test && test.response.body && test.response.body.schema) {
            if (typeof test.response.body.schema == "string") {
                if (test.response.body.schema in this.schemas) {
                    ...
                    new Exceptions_1.NodeError('schema ' + test.response.body.schema + ' not found on test ' + test.method + ' ' + test.uri, test.response.body.lowLevelNode);
                }
            }
        }
    };
        ...
        raml: function (atl, node) {
            if (YAML_1.YAMLAstHelpers.ensureInstanceOf(node, String)) {
                ...
                try {
                    ...
                    new Exceptions_1.NodeError("raml file must have baseUri", node);
                }
            ...
            if (YAML_1.YAMLAstHelpers.ensureInstanceOf(node, Object)) {
                ...
                for (var sequenceName in tests) {
                    if (YAML_1.YAMLAstHelpers.isMap(tests[sequenceName])) {
                        ...
                        new Exceptions_1.NodeError("suites must be non-empty maps: " + sequenceName, node);
                    }
                }
            }
        },
                ...
                if (schemas) {
                    for (var sequenceName in schemas) {
                        ...
                        if (YAML_1.YAMLAstHelpers.isInclude(schemas[sequenceName])) {
                            ...
                            var content = atl.options.loadAssets ? include.content(atl.options.FSResolver) : '{}';
                            try {
                                ...
                                new Exceptions_1.NodeError("error adding schema " + sequenceName + ":" + include.path + ". " + e.toString(), schemas[sequenceName]);
                            }
                        }
                        else {
                            new Exceptions_1.NodeError("schemas: invalid schema " + sequenceName + ", it must be a !include reference", schemas[sequenceName]);
                        }
                    }
                }
            }
